...
function painteco_scripts() {
	...
	wp_enqueue_style( 'painteco-style-fonts',
		'https://fonts.googleapis.com/css?family=Open+Sans:400,700,300&subset=latin,latin-ext' );
	wp_enqueue_style( 'painteco-style-fa',
		'https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css' );
	wp_enqueue_style( 'painteco-style-fancybox',
		get_template_directory_uri() . '/vendor/fancybox/jquery.fancybox.css' );
	...
	wp_enqueue_script( 'painteco-mixitup', 'https://cdn.jsdelivr.net/jquery.mixitup/latest/jquery.mixitup.min.js',
		array( 'jquery' ), '2.1.11', true );
	wp_enqueue_script( 'painteco-slick', 'https://cdn.jsdelivr.net/jquery.slick/1.5.9/slick.min.js', array( 'jquery' ),
		'1.5.9', true );

	wp_enqueue_script( 'painteco-fancybox', get_template_directory_uri() . '/vendor/fancybox/jquery.fancybox.pack.js',
		array( 'jquery' ), '2.1.5', true );

	wp_enqueue_script( 'painteco-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215',
		true );
	wp_enqueue_script( 'painteco-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js',
		array(), '20151215', true );
	wp_enqueue_script( 'painteco-bootstrap', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js',
		array( 'jquery' ), '20160808', true );
	wp_enqueue_script( 'painteco-language', get_template_directory_uri() . '/js/language-switch.js', array( 'jquery' ),
		'20160319', true );
	wp_enqueue_script( 'painteco-calculator', get_template_directory_uri() . '/js/consumption-calculator.js',
		array( 'jquery' ), '20160319', true );
	wp_enqueue_script( 'painteco-colorpicker', get_template_directory_uri() . '/js/color-picker.js', array( 'jquery' ),
		'20160329', true );
	wp_enqueue_script( 'painteco-site', get_template_directory_uri() . '/js/site.js', array( 'jquery' ), '20160808',
		true );
...
function painteco_contact_form() {
	if ( isset( $_POST["form_name"] ) && "Jaunumu forma" == $_POST['form_name'] ) {
		...
		if ( is_email( $client_email ) !== false ) {
			$subject = '=?UTF-8?B?' . base64_encode( "Jauns e-pasts no painteco.com" ) . '?=';
			$message = "Forma: {$form_name}<br\> Email: {$client_email}";
