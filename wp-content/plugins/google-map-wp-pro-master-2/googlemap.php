...
function huge_it_google_maps_shortcode( $atts ) {
	...
	if ( $getMapLang != "location based" ) {
		wp_enqueue_script( "gmap1", 'https://maps.googleapis.com/maps/api/js?'.$key_param.'v=3.exp&sensor=false&libraries=places&language=' . $getMapLang );
	} else {
		wp_enqueue_script( "gmap2", 'https://maps.googleapis.com/maps/api/js?'.$key_param.'v=3.exp&sensor=false&libraries=places' );
	}
