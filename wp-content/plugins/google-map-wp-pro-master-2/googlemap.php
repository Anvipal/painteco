...
function add_google_map_inline_popup_content() {
    ...
    <div id="huge_it_google_map" style="display:none;">
        ...
        <form method="post"
            ...
            <select title="" id='huge_it_map_select'>
				<?php
...
function hugeitgooglemaps_options_panel() {
	$page_cat = add_menu_page( 'Theme page title', 'Google Maps', 'manage_options', 'hugeitgooglemaps_main',
		'hugeitgooglemaps_main', plugins_url( 'images/google-maps-20-x-20.png', __FILE__ ) );
	...
	add_submenu_page( 'hugeitgooglemaps_main', 'Featured Plugins', 'Featured Plugins', 'manage_options',
		'huge_it__google_map_plugins', 'huge_it__google_map_featured_plugins' );
...
function hugeitgooglemaps_admin_script() {
	...
	wp_enqueue_script( "gmap",
		'https://maps.googleapis.com/maps/api/js?' . $key_param . 'v=3.exp&sensor=false&libraries=places' );
	wp_enqueue_script( "simple_slider", plugins_url( "js/simple-slider.js", __FILE__ ), false );
	...
	wp_localize_script( 'my_ajax_script', 'ajax_object', $js_vars );
...
function hg_gmaps_api_key_notice() {
    ...
    <div id="hg_gmaps_no_api_key_big_notice" class="error">
        <p class="hg_mui_heading">Attention!</br>Before you begin using Google Map plugin, please note that All Google
            Maps users now required to have an API key to function. You can read more about that <a
                    href="https://googlegeodevelopers.blogspot.am/2016/06/building-for-scale-updates-to-google.html"
                    target="_blank">here.</a></p>
        <div><a class="hg_mui_btn hg_mui_btn_raised_blue" target="_blank"
                href="https://console.developers.google.com/flows/enableapi?apiid=maps_backend,geocoding_backend,directions_backend,distance_matrix_backend,elevation_backend&keyType=CLIENT_SIDE&reusekey=true">Register
                for Google Maps API now</a></div>
        <p class="hg_mui_heading">Once registered, simply paste your API key here and press the save button. It will
            activate in 5-10 minutes.</p>
        <div>
            <form action="" method="post">
                <label class="hg_mui_text">
                    <span class="hg_mui_label mui_label_mt11">API KEY</span>
                    <div class="hg_mui_input_block">
                        <input name="hg_gmaps_api_key_input" class="hg_gmaps_api_key_input" value="" required="required"
                               type="text"><span class="control_title">Input the api key here</span>
                        <div class="hg_mui_bar"></div>
                    </div>
                </label>
                <div class="hg_gmaps_apply_action">
                    <button class="hg_gmaps_save_api_key_button hg_mui_btn hg_mui_btn_raised_green">Save</button>
                    <span class="spinner"></span></div>
            </form>
        </div>
        <p class="hg_mui_heading">Need help? <a href="http://huge-it.com/contact-us/" target="_blank">Contact Us</a> and
            we will help you with installation.</p>
    </div>
	<?php
}
...
function huge_it_google_maps_shortcode( $atts ) {
	...
	if ( $getMapLang != "location based" ) {
		wp_enqueue_script( "gmap1",
			'https://maps.googleapis.com/maps/api/js?' . $key_param . 'v=3.exp&sensor=false&libraries=places&language=' . $getMapLang );
	} else {
		wp_enqueue_script( "gmap2",
			'https://maps.googleapis.com/maps/api/js?' . $key_param . 'v=3.exp&sensor=false&libraries=places' );
	}
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "ajax" ) {
			...
			if ( isset( $getMap ) ) {
				...
				$sql        = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}.g_markers WHERE map=%s ORDER BY {$wpdb->prefix}.g_markers.city DESC, {$wpdb->prefix}.g_markers.id ASC", $id );
...
function g_map_options_callback() {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "styling_submit" ) {
			...
			$sql = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_maps SET styling_lightness=%d, styling_hue='%s', styling_gamma=%d, styling_saturation=%d WHERE id=%d",
				$_POST['g_map_styling_lightness'], $_POST['g_map_styling_hue'], $_POST['g_map_styling_gamma'],
				$_POST['g_map_styling_saturation'], $_POST['id'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['id'] ) );
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "submit_circle_edit" ) {
			...
			$sql = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_circles SET hover_line_color=%s,hover_line_opacity=%s,hover_fill_color=%s,hover_fill_opacity=%s,name=%s,center_lat=%s, center_lng=%s,radius=%s,line_width=%s,line_color=%s,line_opacity=%s,fill_color=%s,fill_opacity=%s,show_marker=%s WHERE id=%d",
				$_POST['hover_circle_edit_line_color'], $_POST['hover_circle_edit_line_opacity'],
				$_POST['hover_circle_edit_fill_color'], $_POST['hover_circle_edit_fill_opacity'],
				$_POST['circle_edit_name'], $_POST['circle_edit_center_lat'], $_POST['circle_edit_center_lng'],
				$_POST['circle_edit_radius'], $_POST['circle_edit_line_width'], $_POST['circle_edit_line_color'],
				$_POST['circle_edit_line_opacity'], $_POST['circle_edit_fill_color'],
				$_POST['circle_edit_fill_opacity'], $_POST['circle_edit_marker_show'], $_POST['id'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['map_id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "submit_circle" ) {
			...
			$sql = $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "g_circles (map, name, center_lat, center_lng, radius ,hover_line_color ,hover_line_opacity ,hover_fill_color ,hover_fill_opacity , line_width, line_color, line_opacity, fill_color, fill_opacity, show_marker) VALUES(%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%d)",
				$_POST['id'], $_POST['circle_name'], $_POST['circle_center_lat'], $_POST['circle_center_lng'],
				$_POST['circle_radius'], $_POST['hover_circle_line_color'], $_POST['hover_circle_line_opacity'],
				$_POST['hover_circle_fill_color'], $_POST['hover_circle_fill_opacity'], $_POST['circle_line_width'],
				$_POST['circle_line_color'], $_POST['circle_line_opacity'], $_POST['circle_fill_color'],
				$_POST['circle_fill_opacity'], $_POST['circle_marker_show'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "polyline_edit_submit" ) {
			...
			$sql = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_polylines SET name=%s, data=%s ,hover_line_color=%s,hover_line_opacity=%s, line_opacity=%s, line_color=%s,line_width=%d WHERE id=%d",
				$_POST['polyline_edit_name'], $_POST['polyline_edit_coords'], $_POST['hover_polyline_edit_line_color'],
				$_POST['hover_polyline_edit_line_opacity'], $_POST['polyline_edit_line_opacity'],
				$_POST['polyline_edit_line_color'], $_POST['polyline_edit_line_width'], $_POST['id'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['map_id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "submit_polyline" ) {
			...
			$sql = $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "g_polylines (map,name,data,hover_line_color,hover_line_opacity,line_opacity,line_color ,line_width) VALUES (%d,%s,%s,%s,%s,%s,%s,%s)",
				$_POST['id'], $_POST['polyline_name'], $_POST['polyline_coords'], $_POST['hover_polyline_line_color'],
				$_POST['hover_polyline_line_opacity'], $_POST['polyline_line_opacity'], $_POST['polyline_line_color'],
				$_POST['polyline_line_width'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "submit_polygon_edit" ) {
			...
			$sql = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_polygones SET url=%s, hover_line_opacity=%s,hover_line_color=%s,hover_fill_opacity=%s,hover_fill_color=%s,  name=%s, data=%s, line_opacity=%s, line_color=%s, line_width=%s, fill_opacity=%s, fill_color=%s WHERE id=%s",
				$_POST['polygone_edit_url'], $_POST['hover_polygone_edit_line_opacity'],
				$_POST['hover_polygone_edit_line_color'], $_POST['hover_polygone_edit_fill_opacity'],
				$_POST['hover_polygone_edit_fill_color'], $_POST['polygone_edit_name'], $_POST['polygone_edit_coords'],
				$_POST['polygone_edit_line_opacity'], $_POST['polygone_edit_line_color'],
				$_POST['polygone_edit_line_width'], $_POST['polygone_edit_fill_opacity'],
				$_POST['polygone_edit_fill_color'], $_POST['id'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['map_id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "submit_polygon" ) {
			...
			$sql = $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "g_polygones (map , name, url , data ,hover_line_opacity ,hover_line_color,hover_fill_opacity ,hover_fill_color  , line_opacity , line_color , fill_opacity , fill_color, line_width) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
				...
				$_POST['hover_polygone_line_opacity'], $_POST['hover_polygone_line_color'],
				$_POST['hover_polygone_fill_opacity'], $_POST['hover_polygone_fill_color'],
				$_POST['polygone_line_opacity'], $_POST['polygone_line_color'], $_POST['polygone_fill_opacity'],
				$_POST['polygone_fill_color'], $_POST['polygone_line_width'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "submit_marker" ) {
			...
			$sql = $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "g_markers (map,title, animation,lat, lng, description, img, size) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
				$_POST['id'], $_POST['marker_title'], $_POST['marker_animation'], $_POST['marker_location_lat'],
				$_POST['marker_location_lng'], $_POST['marker_description'], $_POST['marker_image'],
				$_POST['marker_image_size'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "submit_general_options" ) {
			...
			$sql = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_maps SET name=%s,type=%s,zoom=%s,border_radius=%s,center_lat=%s,center_lng=%s,pan_controller=%s,zoom_controller=%s,type_controller=%s,scale_controller=%s,street_view_controller=%s,overview_map_controller=%s,width=%s,height=%s,align=%s,info_type=%s,wheel_scroll=%s,draggable=%s,language=%s,min_zoom=%s,max_zoom=%s,animation=%s WHERE id=%s",
				$_POST['map_name'], $_POST['map_type'], $_POST['map_zoom'], $_POST['map_border_radius'],
				$_POST['map_center_lat'], $_POST['map_center_lng'], $_POST['map_controller_pan'],
				$_POST['map_controller_zoom'], $_POST['map_controller_type'], $_POST['map_controller_scale'],
				$_POST['map_controller_street_view'], $_POST['map_controller_overview'], $_POST['map_width'],
				$_POST['map_height'], $_POST['map_align'], $_POST['map_infowindow_type'], $_POST['wheel_scroll'],
				$_POST['draggable'], $_POST['map_language'], $_POST['min_zoom'], $_POST['max_zoom'],
				$_POST['map_animation'], $_POST['id'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "submit_layers" ) {

			global $wpdb;
			$sql = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_maps SET traffic_layer=%s, bike_layer=%s, transit_layer=%s WHERE id=%s",
				$_POST['traffic'], $_POST['bike'], $_POST['transit'], $_POST['id'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['id'] ) );
				foreach ( $map_params as $param ) {
	...
	if ( isset( $_POST['my_task'] ) ) {
		if ( $_POST['my_task'] == "map_styles_set_default" ) {
			global $wpdb;
			$sql = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_maps SET styling_lightness=%s, styling_hue=%s, styling_gamma=%s, styling_saturation=%s WHERE id=%d",
				$_POST['map_lightness'], $_POST['map_hue'], $_POST['map_gamma'], $_POST['map_saturation'],
				$_POST['id'] );
			if ( $wpdb->query( $sql ) ) {
				$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s",
					$_POST['id'] ) );
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "change_name" ) {
			...
			$name = $_POST['name'];
			$id   = $_POST['id'];
			if ( $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_maps SET name='$name' WHERE id=%s",
				$id ) ) ) {
				echo json_encode( array( "success" => 1 ) );
				die();
			}
	...
	if ( isset( $_POST['task'] ) ) {
		if ( $_POST['task'] == "front_end_submit" ) {
			...
			$id     = $_POST['id'];
			$align  = $_POST['align'];
			$sql    = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_maps SET name=%s, type=%d, zoom = %d, width=%d, height=%d, align=%d WHERE id=%d",
		...
		if ( $_POST['task'] == "copy_map" ) {
			...
			if ( $id != "" ) {
				...
				if ( $wpdb->query( $insertQuery ) ) {
					...
					$queryMarkers  = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}.g_markers WHERE map=%s ORDER BY {$wpdb->prefix}.g_markers.city DESC, {$wpdb->prefix}.g_markers.id ASC", $id );
					$markerResults = $wpdb->get_results( $queryMarkers );
					if ( $markerResults ) {
						foreach ( $markerResults as $markercopy ) {
							$wpdb->query( $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "g_markers (map,title,lat,lng,animation,description,img,size) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s')",
								$new_map_id, $markercopy->title, $markercopy->lat, $markercopy->lng,
								$markercopy->animation, $markercopy->description, $markercopy->img,
								$markercopy->size ) );
						}
					}

					$queryPolygons  = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_polygones WHERE map=%s",
						$id );
					...
					if ( $polygonResults ) {
						...
						",
								$new_map_id, $polygonCopy->name, $polygonCopy->data, $polygonCopy->line_opacity,
								$polygonCopy->line_color, $polygonCopy->fill_opacity, $polygonCopy->fill_color,
								$polygonCopy->url, $polygonCopy->hover_line_opacity, $polygonCopy->hover_line_color,
								$polygonCopy->hover_fill_opacity, $polygonCopy->hover_fill_color,
								$polygonCopy->line_width
							) );
						}
					}

					$queryPolylines  = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_polylines WHERE map=%s",
						$id );
					...
					if ( $polylineResults ) {
						foreach ( $polylineResults as $polylineCopy ) {
							...
							('%s','%s','%s','%s','%s','%s','%s','%s')
							",
								$new_map_id, $polylineCopy->name, $polylineCopy->data, $polylineCopy->line_opacity,
								$polylineCopy->line_color, $polylineCopy->line_width, hover_line_color,
								$polylineCopy->hover_line_opacity
							) );
						}
					}
					...
					if ( $circleResults ) {
						foreach ( $circleResults as $circleCopy ) {
							...
							",
								$new_map_id, $circleCopy->name, $circleCopy->center_lat, $circleCopy->center_lng,
								$circleCopy->radius, $circleCopy->line_width, $circleCopy->line_opacity,
								$circleCopy->line_color, $circleCopy->fill_color, $circleCopy->fill_opacity,
								$circleCopy->hover_line_opacity, $circleCopy->hover_line_color,
								$circleCopy->hover_fill_color, $circleCopy->hover_fill_opacity, $circleCopy->show_marker
							) );
						}
					}
		...
		if ( $_POST['task'] == "export_to_csv" ) {
			...
			if ( $markerResults ) {
				...
				foreach ( $markerResults as $marker ) {
					array_push( $map_array,
						'ID: ' . $marker->id . ', title: ' . $marker->title . ', latitude: ' . $marker->lat . ', longitude:' . $marker->lng . ', animation:' . $marker->animation . ', description:' . $marker->description . ', image:' . $marker->img . '' );
				}
			}
			...
			if ( $polylineResults ) {
				...
				foreach ( $polylineResults as $polyline ) {
					array_push( $map_array,
						'name :' . $polyline->name . ', data :' . $polyline->data . ', line transparency :' . $polyline->line_opacity . ', line color :' . $polyline->line_color . ', line width :' . $polyline->line_width . ', hover line color :' . $polyline->hover_line_color . ', hover line transparency :' . $polyline->name . '' );
				}
			}
...
class Huge_it_google_maps_Widget extends WP_Widget {
	...
	public function form( $instance ) {
        ...
        <label
                for="<?php echo $this->get_field_id( 'g_map_id' ); ?>"><?php _e( 'Select map:',
				'huge_it_google_maps' ); ?></label>
...
function huge_it_google_maps_activate() {
	...
	$sqlInsertMarker1 = "INSERT INTO " . $wpdb->prefix . "g_markers (map,title,lat,lng,animation,description) VALUES ('1', 'New York', '40.7127837', '-74.00594130000002', 'BOUNCE', '{city: \"New York City\"}')";

	$sqlInsertMarker2 = "INSERT INTO " . $wpdb->prefix . "g_markers (map,title,lat,lng,animation,description) VALUES ('1', 'Delaver', '39.189690821096804', '-75.7562255859375', 'DROP', '{city: \"Delaver\"}')";
